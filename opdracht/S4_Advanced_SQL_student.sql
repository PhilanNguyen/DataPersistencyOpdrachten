...
SELECT mnr, functie, gbdatum FROM medewerkers WHERE gbdatum < '1980-01-01' AND functie='TRAINER' OR gbdatum < '1980-01-01' AND functie='VERKOPER'
...
SELECT u.cursus, u.begindatum, COUNT(i.cursist)
FROM uitvoeringen u JOIN inschrijvingen i ON u.cursus = i.cursus AND u.begindatum = i.begindatum
WHERE u.begindatum BETWEEN '2019-01-01' AND '2019-12-31'
GROUP BY u.cursus, u.begindatum HAVING COUNT(i.cursist) >= 3
...
SELECT cursus, COUNT(cursus) as aantal FROM uitvoeringen GROUP BY cursus
...
SELECT EXTRACT(YEAR FROM AGE(MIN(gbdatum))) - EXTRACT(YEAR FROM AGE(MAX(gbdatum))) AS verschil,
ROUND(AVG(EXTRACT(YEAR FROM AGE(gbdatum))), 2) AS gemiddeld FROM medewerkers;
...
SELECT COUNT(*) AS aantal_medewerkers, SUM(comm)/ COUNT(*) AS comissie_medewerkers,
       SUM(comm)/ COUNT(comm) AS comissie_verkoper
FROM medewerkers;
-- -------------------------[ HU TESTRAAMWERK ]--------------------------------
